...
DATA data;
map<long,DATA> mydata;
map<long,DATA>::iterator it;
...
void printSubarray(int a[]){			//输出数组最大子序列 
	...
	for(it = mydata.begin(); it != mydata.end(); it++){
		...
		cout<<"数组下标："<<"["<<it->second.left<<",";
		cout<<it->second.right<<"]"<<"   ";
		cout<<"子数组为："; 
...
long maxSubSum(int a[], int left, int right){	//求a[left..right]序列中的最大连续子序列和 
	...
	if(left == right){							//当子序列只有一个元素时 
		if(a[left] > 0){						//该元素大于0时返回它
			...
			mydata[sum] = data;			//将子数组的和与结构体（数组左右下标）数据放入set 
			return a[left];
		}
		else									//该元素小于或等于0时返回0 
			return 0;
...
/*
	...
	1..map容器应该用multimap,既最大序列的和可能有相等的多个. 
	2..输出最大序列时只需要从容器中提取mydata.end()-1位置的.second().left&.second().right(),但现在无法提取
	本程序全部提取,答案为只看最后的一行,但中间的序列存在错误,最后一个确是正确. 
*/
